...
app.post("/cadastroPratoCozinha", function (req, res) {
  ...
  let sql = `INSERT INTO Cozinha (nome, link, observacao, preco, categoria, id_restaurante, id_mesa, estado) VALUES ("${nome}", "${link}", "${observacao}", "${preco}", "${categoria}", "${id_restaurante}", "${id_mesa}", "${estado}")`;
  db.query(sql, [], (err, rows) => {
    if (err) {
      console.log("Erro" + err);
      res.send(err);
...
app.post("/cozinhaWeb", function (req, res) {
  ...
  let sql = `SELECT * FROM Cozinha WHERE id_restaurante="${id_restaurante}" AND estado="prod"`;
  db.query(sql, [], (err, rows) => {
    if (err) {
      console.log("Erro" + err);
      res.send(err);
